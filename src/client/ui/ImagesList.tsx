...
class ImagesList extends React.Component<Props, State> {
  ...
  loadImagesComplete = (data: LoadedImages = {}) => {
    ...
    if (names.length) {
      ...
      for (const name of names) {
        ...
        if (!img) continue;
        if (!img.rect) img.rect = getDummyRect(name, img.width, img.height);
        rects.push(img.rect);
        /*images[name] = {
					image: data[name],
					name: name,
  ...
  getFirstImageInFolder = (path: string) => {
    ...
    for (const key in images) {
      if (key.substring(0, path.length + 1) === path + "/") return images[key];
    }
  ...
  getLastImageInFolder = (path: string) => {
    ...
    let ret = null;
    for (const key in images) {
      if (key.substring(0, path.length + 1) === path + "/") ret = images[key];
    }
  ...
  renderWebButtons() {
    return (
      <span className="images-upload-buttons">
        <div
  ...
  override render() {
    ...
    let dropHelp =
      Object.keys(this.state.images).length > 0 ? null : (
        <div ref={this.dropHelpRef} className="image-drop-help">
